...
startup_growing_tax <- read.csv("~/Documentos/Source-Python/startupbr/src/datamining/csv-en/analysis/startup_growing_tax_by_city.csv", sep=";")
...
df %>%
  ...
  ggplot(aes(x = Region, y = QuantityStartups, fill = Region)) +
    geom_boxplot() + xlab("Region") + scale_y_continuous(name = "Number of startups", trans = "log10") + labs(fill = "Region") +
